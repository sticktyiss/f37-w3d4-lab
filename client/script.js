...
sayHelloButton.addEventListener('mouseover', toBlack)
// PROBLEM 3
/*
...
sayHelloButton.addEventListener('mouseout', toWhite)
...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/request')
    .then((res) => {
...
// CODE HERE
const queryBtn = () => {
    axios.get('http://localhost:3000/query-test?name=lichee')
    .then((res) => {
        console.log(res.data)
    })
    .catch((err) => {
    ...
    axios.get('http://localhost:3000/query-test?anime=awesome')
    .then((res) => {
        console.log(res.data)
    })
    .catch((err) => {
...
/*
    ...
    Had to send the queries separately.
...
function createFoodItem(food) {
    ...
    foodItem.innerHTML = `<li>${food}</li>`

    document.querySelector('ol').appendChild(foodItem)
}


createFood = (event) => {
    event.preventDefault()
    foodList = document.createElement('ol')
    document.querySelector('form').appendChild(foodList)
...
//QUESTION: how does the /food and /query-test even exist